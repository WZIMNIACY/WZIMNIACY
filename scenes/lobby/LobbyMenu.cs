...
public partial class LobbyMenu : Control
{
	...
	public override void _Ready()
	{
		...
		// Pobieramy nowy manager
		networkManager = GetNode<NetworkManager>("/root/NetworkManager");
		eosManager.GameStartedFromLobby += OnGameStartedFromLobby;
	...
	private void OnStartGamePressed()
	{
		...
		eosManager.SetLobbyAttribute("GameStatus", "Starting");
		GD.Print("🎮 Starting game...");
		GetTree().ChangeSceneToFile("res://scenes/game/main_game.tscn");
	}
	
	private void OnGameStartedFromLobby()
{
	GD.Print("🔔 Odebrano sygnał startu gry z Lobby!");
	if (eosManager.isLobbyOwner) return;
	string hostId = eosManager.GetCurrentLobbyOwnerId();
	if (string.IsNullOrEmpty(hostId))
	{
		GD.PrintErr("❌ Błąd: Nie znaleziono ID Hosta!");
		return;
	}
	if (networkManager != null)
	{
		networkManager.StartClient(hostId);
	}
	GetTree().ChangeSceneToFile("res://scenes/game/main_game.tscn");
}
